...
import decimal
import math
sys.path.append('/Users/sg00509ml/ApeX/openapi/')
...
def apexGetRequest(apiKey,secretKey,passPhrase,domainName,endpoint,**kwargs): 
    ...
    headers = {
            ...
            'APEX-API-KEY': apiKey,
            'APEX-TIMESTAMP': str(int(time.time()*1000)),
            'APEX-PASSPHRASE': passPhrase
    }
    for x in range(1):
        response = requests.get(domainName+endpointQueryString, headers=headers,verify=False) # You must specify "Content-Type" to "application/x-www-form-urlencoded" if you put query string into request body
        return json.loads(response.text)


def apexPostRequest(apiKey,secretKey,passPhrase,domainName,endpoint,**kwargs):
    timeStamp=str(int(time.time()*1000))
    params = kwargs
    queryString = ''
    ...
    response = requests.post(domainName+endpoint, data=params, headers=headers) # You must specify "Content-Type" to "application/x-www-form-urlencoded" if you put query string into request body
    #print(response)
    print(response.text)
...
def main():
    ...
    secret = 'xxxx'
    passphrase = 'xxxx'
    #resp=apexGetRequest(apiKey,secret,passphrase,"https://qa.pro.apex.exchange","/api/v1/account")
    resp=apexGetRequest(apiKey,secret,passphrase,"https://pro.apex.exchange","/api/v1/account")
    symbol="BTC-USDC"
    symbolInfo=list(filter(lambda x:x["symbol"]==symbol,json.loads(requests.get("https://pro.apex.exchange/api/v1/symbols").text)["data"]["perpetualContract"]))[0]
    currency="USDC"
    currencyInfo=list(filter(lambda x:x["id"]==currency,json.loads(requests.get("https://pro.apex.exchange/api/v1/symbols").text)["data"]["currency"]))[0]
    accountInfo=resp["data"]
    side="BUY"
    size="2"
    price=float(sys.argv[1])
    #clientId="00001146"
    clientOrderId="00001167-"+str(randint(1,1000000))
    limitFee=accountInfo["takerFeeRate"]
    expirationEpochSeconds=time.time()+60
    ONE_HOUR_IN_SECONDS = 60 * 60
    ORDER_SIGNATURE_EXPIRATION_BUFFER_HOURS = 12 * 7 *4
    quantums_amount_synthetic = to_quantums_exact(size,symbolInfo['starkExResolution'])
    ...
    limit_fee_rounded_4_hash = DECIMAL_CONTEXT_ROUND_UP.quantize(decimal.Decimal(limitFee),decimal.Decimal(currencyInfo['stepSize']))
    ...
    synthetic_asset_id = int(
            symbolInfo['starkExSyntheticAssetId'],
            16,
    )
    collateral_asset_id = int(
            currencyInfo['starkExAssetId'],
            16,
    )
    ...
    for _ in range(3):
        ...
        part_2 += int(accountInfo["positionId"])
    ...
    r, s = py_sign(msg_hash=signHash, priv_key=int(private_key, 16))
    signature=serialize_signature(r, s)
    limit_fee_rounded = DECIMAL_CONTEXT_ROUND_UP.quantize(decimal.Decimal(fee),decimal.Decimal(currencyInfo['stepSize']))
    ...
    order = {'symbol': symbol,'side': side,'type': "LIMIT",'timeInForce': "GOOD_TIL_CANCEL",'size': str(size),'price': str(price),'limitFee': str(limit_fee_rounded),'expiration': expirationEpoch * 3600 * 1000,'clientOrderId': clientOrderId,'signature': signature,'reduceOnly':False}
    apexPostRequest(apiKey,secret,passphrase,"https://pro.apex.exchange","/api/v1/create-order",**order)
